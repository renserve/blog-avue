...
export default {
  ...
  created() {
    ...
    // 登录请求
    if (type === 'LOGIN') {
      window.opener.location.href = `${window.baseURL}/#/login?state=${state}&code=${code}&time=` + new Date().getTime()
      // window.opener.location.href = `http://192.168.8.100:9999/#/login?state=${state}&code=${code}&time=` + new Date().getTime()
      window.close()
    } else {
