...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            deleteAndBlack(form){
                //拉黑前查询文章ID(classId),判断是否是作者本人拉黑
                form.remark=form.content
                this.rowTagClick({...form,$sign:1,id:this.baseDialogConfig.sendData.classId},true,`拉黑邮箱为${form.email}的用户`)
            },
            rowTagApi(data){
                if(data.$sign){
                    delete data.$sign
                    return new Blacklist().postAndDeleteComment(data)
                }else {
