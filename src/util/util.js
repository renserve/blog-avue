...
//生成访问菜单
export function recursionMenu(parent,children,records=[]) {
    parent.map(parentItem=>{
        children.map((childrenItem,index)=>{
            if(childrenItem.parentId===parentItem.id){
                if(childrenItem.type==='button'){
                    parentItem.meta.hasButton=true
                }
    ...
    if(parent.filter(i=>i.children)[0]){
        //有下级继续递归
        recursionMenu(children,differenceBy(children,records,'id'),records)
    }
    return parent
}
// 表单序列化
...
export const encryption = (params) => {
    ...
    if (type === 'Base64') {
        ...
        param.forEach(ele => {
            ...
            ivKey = CryptoJS.enc.Latin1.parse(ivKey)
            // 加密
