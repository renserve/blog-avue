...
const common = {
    state: {
        avueConfig: {
            size: getStore({name: 'avueConfig_size'}) || 'mini',
            span: getStore({name: 'avueConfig_span'}) || 24,
            danger: getStore({name: 'avueConfig_danger'}) || 'danger',
        },
        wordcloudConfig: {
            minFontSize: getStore({name: 'wordcloudConfig_minFontSize'}) || 30,
            maxFontSize: getStore({name: 'wordcloudConfig_maxFontSize'}) ||  80,
            split:  getStore({name: 'wordcloudConfig_split'}) || 30,
            rotate: getStore({name: 'wordcloudConfig_rotate'}) ||  true
        },
        isOpenCloud:getStore({name: 'wordcloudConfig_isOpenCloud'}),
        cloudCount:getStore({name: 'wordcloudConfig_cloudCount'}) || 50,
        otherCloud:getStore({name: 'wordcloudConfig_otherCloud'}) || false,
        isCollapse: false,
        isFullScren: false,
        isShade: false,
        screen: -1,
    ...
    mutations: {
        SET_WEL: (state, data) => {
            state.wordcloudConfig[data[0]] = data[1]
            setStore({
                name: `wordcloudConfig_${data[0]}`,
                content: data[1]
            })
        },
        SET_CONFIG: (state, data) => {
            state.avueConfig[data[0]] = data[1]
            setStore({
                name: `avueConfig_${data[0]}`,
                content: data[1]
            })
        },
        ...
        SET_WEL_CLOUD_OPEN: (state,isOpenCloud) => {
            state.isOpenCloud = isOpenCloud
                setStore({
                    name: 'wordcloudConfig_isOpenCloud',
                    content: state.isOpenCloud
                })
        },
        SET_WEL_CLOUD_COUNT: (state, count) => {
            state.cloudCount = count
            setStore({
                name: 'wordcloudConfig_cloudCount',
                content: state.cloudCount
            })
        },
        SET_WEL_CLOUD: (state, active) => {
            state.otherCloud = active
            setStore({
                name: 'wordcloudConfig_otherCloud',
                content: state.otherCloud
            })
        },
        SET_FULLSCREN: (state) => {
